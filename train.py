...
def _initialize_model_from_pretrained_boq(hparams, model):
    ...
    if hparams.use_kitti360_boq and hparams.shared_query_boq:
    ...
    if hparams.use_kitti360_boq and getattr(model, "is_view_specific_adapter_encoder", False):
    ...
    if hparams.use_kitti360_boq and getattr(model, "is_dual_branch", False):
...
def train(hparams, dev_mode=False):
    ...
    if hparams.use_kitti360_boq:
        ...
        if not hparams.kitti360_path:
            raise ValueError("Please set --kitti360_path when using --use_kitti360_boq.")

        use_view_specific_adapter = bool(hparams.use_view_specific_adapter)
        use_dual_branch = hparams.dual_branch or hparams.shared_query_boq
        if use_view_specific_adapter and use_dual_branch:
            raise ValueError(
                ...
                "The new model already uses a single shared backbone."
            )
    ...
    if hparams.compile and not hparams.use_kitti360_boq:
    ...
    if not hparams.silent:
        if hparams.use_pretrained_boq and pretrained_boq_spec is not None:
            ...
            if changed_pretrained_hparams:
                changed_summary = ", ".join(
                    f"{name}:{old}->{new}" for name, old, new in changed_pretrained_hparams
                )
        ...
        if hparams.use_kitti360_boq:
            if hparams.shared_query_boq:
                print(
                    "[KITTI360][SharedQueryBoQ]",
                    f"query_backbone={hparams.query_backbone}",
                    f"db_backbone={hparams.db_backbone}",
                    f"adapter_out_channels={hparams.adapter_out_channels}",
                    f"num_layers={hparams.num_layers}",
                    f"num_queries={hparams.num_queries}",
                    f"slot_loss_weight={hparams.slot_loss_weight}",
                    f"ranking_loss={hparams.ranking_loss_type or ('hinge' if hparams.loss_type == 'triplet' else hparams.loss_type)}",
                    f"backbone_warmup_epochs={hparams.backbone_warmup_epochs}",
                )
            elif use_view_specific_adapter:
                print(
                    "[KITTI360][ViewSpecificAdapter]",
                ...
                print("[KITTI360][ViewSpecificAdapter] adapter_status:")
                for line in model.image_encoder.format_adapter_status_lines():
                    print(f"  {line}")
                print("[KITTI360][ViewSpecificAdapter] trainable_params:")
                for line in model.image_encoder.format_trainable_summary_lines():
                    print(f"  {line}")
                print("[KITTI360][ViewSpecificAdapter] module_summary:")
        ...
        if hparams.use_kitti360_boq and not (hparams.shared_query_boq or use_view_specific_adapter or use_dual_branch):
            standard_backbone = model.image_encoder.backbone
        elif not hparams.use_kitti360_boq:
    ...
    if hparams.use_kitti360_boq and hparams.shared_query_boq:
        logger_name = f"shared_query_{hparams.query_backbone}_{hparams.db_backbone}"
    elif hparams.use_kitti360_boq and use_view_specific_adapter:
        logger_name = f"view_adapter_{hparams.backbone_name}"
    elif hparams.use_kitti360_boq and use_dual_branch:
        logger_name = f"dual_{hparams.query_backbone}_{hparams.db_backbone}"
    elif hparams.use_dinov2_block_adapter and "dinov2" in logger_name:
        logger_name = f"{logger_name}_block_adapter"
    tensorboard_logger = TensorBoardLogger(save_dir="./logs", name=logger_name, default_hp_metric=False)
